...
"""
LLaDA Evaluation Adapter for lm-evaluation-harness

Enables LLaDA (Diffusion LLM) evaluation on standard benchmarks.
Uses Monte Carlo estimation for log-likelihood since Diffusion LLMs 
cannot directly compute log P(target | prefix).
...
class LLaDAEvalHarness(LM):
    """
    LLaDA adapter for lm-evaluation-harness.
    Uses Monte Carlo estimation: log P(target|prefix) ≈ -E[loss/mask_ratio]
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Token Skip params (History-skip: T-2 vs T-1)
        token_skip_raw = kwargs.get('token_skip', 'False')
        self.token_skip = str(token_skip_raw).lower() in ('true', '1', 'yes')
        self.skip_threshold = float(kwargs.get('skip_threshold', 0.99))
        self.skip_layers = int(kwargs.get('skip_layers', 8))
        self.force_full_every_k = int(kwargs.get('force_full_every_k', 3))
        
        # Early Exit params
        early_exit_raw = kwargs.get('early_exit', 'False')
        self.early_exit = str(early_exit_raw).lower() in ('true', '1', 'yes')
        self.early_exit_layer = int(kwargs.get('early_exit_layer', 16))
        self.early_exit_threshold = float(kwargs.get('early_exit_threshold', 1.0))
    ...
    def _forward_process(self, batch, prompt_index):
        """
        Forward diffusion: add noise by randomly masking tokens.
        Returns noisy_batch and mask_ratio for weighted loss.
        """
        b, l = batch.shape
    ...
    def get_logits(self, batch, prompt_index):
        """Get model logits, supports CFG (currently unused)."""
        if self.cfg > 0.:
    ...
    def get_loglikelihood(self, prefix, target):
        """
        Estimate log P(target|prefix) using Monte Carlo sampling.
        Returns negative weighted average loss.
        """
    ...
    def suffix_greedy_prediction(self, prefix, target):
        """
        Verify if greedy decoding produces target sequence.
        Slow: one forward pass per target token.
        """
    ...
    def _encode_pair(self, context, continuation):
        """Encode (context, continuation) pair, handling trailing spaces."""
    ...
    def loglikelihood(self, requests):
        """Compute log P(target|prefix) for each request."""
        def _tokenize(e):
    ...
    def generate_until(self, requests):
        """Generate text until stop tokens. Supports batching and checkpointing."""
        output = []
        num_tokens = 0
        num_nfe = 0
        ...
        for batch in tqdm(batched_requests, desc="Generating..."):
            ...
            if self.use_cache:
                if self.dual_cache and self.token_skip:
                    # Dual Cache + Token Skip (History-skip)
            ...
            if self.is_instruct and 'task_id' in req.doc and str(req.doc['task_id']).lower().startswith('humaneval'):
                ...
                # Normal tasks: truncate at stop tokens
                batched_generated_answer = []
